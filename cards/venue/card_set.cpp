#include "cards/venue/card_set.h"
...
// 牌库
Card_Set::Card_Set(): MAX_REMAINDER(60), fatigue(0)  // 构造函数
{
...
{
    if (cardss.size() == 0) // 如果牌库为空，造成疲劳伤害
    {
        ++fatigue; // 疲劳伤害更新
...
unsigned Card_Set::size_card_set()    // 返回剩余卡牌数量
{
    return cardss.size();
}
